...
```bash
helm install example activiti-cloud-helm-charts/activiti-cloud-full-example --version 7.9.0 --set global.gateway.domain=REPLACEME --set global.keycloak.clientSecret=$(uuidgen) --set global.gateway.http=false
```
